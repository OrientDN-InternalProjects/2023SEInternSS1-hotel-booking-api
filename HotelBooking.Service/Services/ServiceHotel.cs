...
namespace HotelBooking.Service.Services
{
    public class ServiceHotel : IServiceHotel
    {
        ...
        public async Task<IEnumerable<ExtraService>> GetAllAsync()
        {
            ...
            var objects = await serviceRepository.GetAllServiceHotelesAsync();
        ...
        {
            var result = await serviceRepository.GetServiceHotelByIdAsync(id);
            return result;
        }

        public async Task<bool> UpdateAsync(UpdateServiceHotelDTO model)
        {
            var result = await serviceRepository.GetServiceHotelByIdAsync(model.Id);
            if (result == null)
            {
                return false;
            }
            ...
            serviceRepository.UpdateServiceHotel(result);
            await unitOfWork.SaveAsync();
            return true;
        }
    }
